...
@section('meta_description')
    <meta name="description" content="This is Error Hero website home page.">
@endsection

@section('meta_keywords')
    <meta name="keywords" content="S M Alif Ahmmed, blog, error hero, error, hero, Laravel, php,">
@endsection
                    ...
                    @foreach($blogs->take(10) as $blog)
                        @if($blog->blog_status == 1)
                            ...
                            <article class="row mb-5">
                                ...
                                <div class="col-md-8">
                                    <h3 class="h5">
                                        <a class="post-title" href="{{ route('details', $blog->slug) }}">{{ $blog->heading }}</a>
                                    </h3>
                                    <ul class="list-inline post-meta mb-2">
                                        ...
                                        <li class="list-inline-item">Categories :
                                            <a href="{{ route('blogs', $blog->category->category_slug) }}" class="ml-1">
                                                {{ $blog->category->name }}
                                            </a>
                                        </li>
                                        <li class="list-inline-item">Tags :
                                            @foreach($blog->tags as $tag)
                                                <a href="{{ route('blog.tag', $tag->tag_slug) }}" class="ml-1">
                                                    {{ $tag->name }}
                                                </a>,
                                            @endforeach
                                        </li>
                                    ...
                                    <a href="{{ route('details', $blog->slug) }}" class="btn btn-outline-primary">Continue Reading</a>
                                </div>
                            </article>
                            @endif
                        @endif
                    ...
                    <div class="widget">
                        ...
                        <ul class="list-unstyled widget-list">
                            @foreach($categories as $category)
                                <li>
                                    <a href="{{ route('blogs', $category->category_slug) }}" class="d-flex">
                                        {{ $category->name }}
                        ...
                        <ul class="list-inline widget-list-inline">
                            @foreach($tags as $tag)
                                <li class="list-inline-item">
                                    <a href="{{ route('blog.tag', $tag->tag_slug) }}">{{ $tag->name }}</a>
                                </li>
                            @endforeach
                        </ul>
                    </div>
